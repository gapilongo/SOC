...
class WorkflowEngine:
    ...
    async def execute_workflow(
        self, alert_id: str, initial_state: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _persist_final_state(self, workflow_state: WorkflowState):
        ...
        try:
            ...
            # Rest of the persistence logic remains the same...
            try:
    ...
    def _route_after_triage(self, state: WorkflowState) -> str:
        ...
        if confidence < 15:  # Very low confidence - close immediately
            return "close"
        elif confidence > 90:  # Very high confidence - might skip correlation for speed
            return "analysis"  # Or "correlation" if you want correlation on everything
        else:
            # ROUTE TO CORRELATION for medium confidence alerts
            return "correlation"
    ...
    def _route_after_correlation(self, state: WorkflowState) -> str:
        ...
        confidence = state["confidence_score"]
        correlation_score = state.get("correlation_score", 0)
        correlation_count = len(state.get("correlations", []))

        # If correlations boosted confidence significantly, route to analysis
        if correlation_score > 70 and correlation_count >= 3:
            return "analysis"
        elif confidence > 85:  # High confidence after correlation
            return "response"
        elif confidence < 25:  # Still low confidence after correlation
            return "close"
        else:
            # Medium confidence - needs deeper analysis
            return "analysis"
