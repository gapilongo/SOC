...
class AnalysisAgent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_fallback_analysis = self.get_config("enable_fallback_analysis", True)
    ...
    def _create_fallback_orchestrator(self):
        ...
        class FallbackOrchestrator:
            ...
            async def execute_tool(self, tool_name: str, tool_args: Dict[str, Any], context: Dict[str, Any] = None):
    ...
    async def _perform_analysis_with_fallbacks(self, alert: Dict[str, Any], correlations: List[Dict[str, Any]], 
        ...
        if self.enable_llm_reasoning and self.react_reasoner and self.tool_orchestrator:
            try:
                result = await self._perform_react_analysis_resilient(alert, correlations, enriched_data, state)
                result["method"] = "react_analysis"
                return result
            except Exception as e:
                self.logger.warning(f"ReAct analysis failed: {e}, trying hybrid approach")
        ...
        if self.enable_llm_reasoning and self.react_reasoner:
            try:
                result = await self._perform_hybrid_analysis(alert, correlations, enriched_data, state)
                result["method"] = "hybrid_analysis"
                return result
            except Exception as e:
                self.logger.warning(f"Hybrid analysis failed: {e}, falling back to rule-based")
                self.tool_failure_count += 1
        
        # Strategy 3: Enhanced rule-based analysis (fallback)
        result = await self._perform_enhanced_rule_based_analysis(alert, correlations, enriched_data, state)
    ...
    async def _execute_single_action_resilient(self, action: Dict[str, str], alert: Dict[str, Any], 
        ...
        for attempt in range(self.tool_retry_attempts + 1):
            try:
                ...
                if self.tool_orchestrator and hasattr(self.tool_orchestrator, 'execute_tool'):
                ...
                result = await self._execute_tool_direct(tool_name, tool_args, alert, state)
                if result and result.get("success"):
                    return result
    ...
    async def _execute_tool_direct(self, tool_name: str, tool_args: Dict[str, Any], 
                                 alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _create_tool_fallback_result(self, tool_name: str, target: str, analysis_type: str) -> Dict[str, Any]:
    ...
    async def _validate_and_enhance_react_result(self, react_result: Dict[str, Any], 
        ...
        final_result = await self._synthesize_final_analysis_enhanced(
            context["alert"], context["correlations"], context["enriched_data"], context["state"]
        )
    ...
    async def _perform_hybrid_analysis(self, alert: Dict[str, Any], correlations: List[Dict[str, Any]], 
                                     enriched_data: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        available_tools = ["rule_based_analysis", "confidence_assessment", "pattern_matching"]
        
        # Run ReAct with rule-based execution
        react_result = await self.react_reasoner.reason_and_act(
            context, available_tools, execute_action_callback=self._execute_rule_based_action
        )
        
        # Enhance with rule-based analysis
        rule_results = await self._perform_enhanced_rule_based_analysis(alert, correlations, enriched_data, state)
        
        # Combine results
        return {
            "confidence_score": max(rule_results["confidence_score"], 60),  # Boost for hybrid
    ...
    async def _execute_rule_based_action(self, action: Dict[str, str], alert: Dict[str, Any], 
                                       state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"success": True, "result": "Rule-based action completed", "confidence": 50}

    async def _perform_enhanced_rule_based_analysis(self, alert: Dict[str, Any], correlations: List[Dict[str, Any]], 
                                                  enriched_data: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        threat_indicators = await self._analyze_threat_indicators_comprehensive(alert, correlations)
        confidence_score = await self._calculate_rule_based_confidence(alert, correlations, threat_indicators)
        threat_score = await self._calculate_threat_score_comprehensive(confidence_score, correlations, threat_indicators)
        
        # Generate conclusion and actions
        conclusion = await self._generate_rule_based_conclusion(confidence_score, threat_score)
        recommended_actions = await self._generate_rule_based_actions(confidence_score, threat_score, alert)
        
        return {
            "confidence_score": confidence_score,
            "conclusion": conclusion,
            "reasoning_history": [{"iteration": 1, "thought": "Rule-based analysis completed"}],
            "tool_results": {"rule_analysis": {"success": True, "indicators": threat_indicators}},
    ...
    async def _analyze_ip_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        """Direct IP analysis implementation."""
        raw_data = alert.get("raw_data", {})
        ip = tool_args.get("target") or raw_data.get("source_ip") or raw_data.get("destination_ip")
    ...
    async def _analyze_hash_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_process_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_network_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_temporal_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_rule_based_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "threat_keywords_found": len([i for i in indicators if "threat_keyword" in i]),
            "severity_assessment": severity
        }

    async def _assess_confidence_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _match_patterns_direct(self, tool_args: Dict[str, Any], alert: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_threat_indicators_comprehensive(self, alert: Dict[str, Any], correlations: List[Dict[str, Any]]) -> List[str]:
        ...
        if raw_data.get("file_path") and "temp" in raw_data.get("file_path", "").lower():
    ...
    async def _calculate_rule_based_confidence(self, alert: Dict[str, Any], correlations: List[Dict[str, Any]], 
                                             threat_indicators: List[str]) -> int:
        ...
        # Severity factor
        severity = alert.get("severity", "").lower()
        severity_boost = {"critical": 25, "high": 20, "medium": 10, "low": 0, "info": -10}.get(severity, 0)
        ...
        final_confidence = max(0, min(100, base_confidence + severity_boost + indicator_boost + correlation_boost + source_penalty))
    ...
    async def _calculate_threat_score_comprehensive(self, confidence: int, correlations: List[Dict[str, Any]], 
                                                  threat_indicators: List[str]) -> int:
        ...
        # Correlation impact
        if correlations:
            critical_correlations = [c for c in correlations if c.get("threat_level") == "critical"]
            high_correlations = [c for c in correlations if c.get("threat_level") == "high"]
            
            threat_score += len(critical_correlations) * 10
            threat_score += len(high_correlations) * 5
        
        # Indicator impact
        critical_indicators = [i for i in threat_indicators if any(critical in i for critical in 
                                                                 ["suspicious_port", "temp_directory", "privileged_account"])]
        threat_score += len(critical_indicators) * 8
        
        return max(0, min(100, threat_score))

    async def _generate_rule_based_conclusion(self, confidence: int, threat_score: int) -> str:
    ...
    async def _generate_rule_based_actions(self, confidence: int, threat_score: int, alert: Dict[str, Any]) -> List[str]:
        ...
        if raw_data.get("source_ip") and not any(raw_data["source_ip"].startswith(p) for p in ["10.", "172.16.", "192.168."]):
            actions.append(f"Consider blocking IP {raw_data['source_ip']}")
        
        return actions

    async def _synthesize_final_analysis_enhanced(self, alert: Dict[str, Any], correlations: List[Dict[str, Any]], 
                                                enriched_data: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        threat_score = await self._calculate_threat_score_comprehensive(final_confidence, correlations, [])
        
        # Generate enhanced conclusion
        conclusion = await self._generate_enhanced_conclusion(final_confidence, threat_score, self.reasoning_history)
        ...
        metadata = {
            ...
            "tools_successful": len([r for r in self.tool_results.values() if r.get("success", False)]),
            "tool_failures": self.tool_failure_count,
    ...
    async def _generate_enhanced_conclusion(self, confidence: int, threat_score: int, 
                                          reasoning_history: List[Dict[str, Any]]) -> str:
        """Generate enhanced analysis conclusion."""
        
        base_conclusion = await self._generate_rule_based_conclusion(confidence, threat_score)
        
        # Add reasoning context if available
        if reasoning_history:
            reasoning_quality = len([r for r in reasoning_history if len(r.get("thought", "")) > 50])
    ...
    async def _generate_enhanced_actions(self, confidence: int, threat_score: int, 
                                       alert: Dict[str, Any], tool_results: Dict[str, Dict[str, Any]]) -> List[str]:
        """Generate enhanced recommended actions."""
        
        actions = await self._generate_rule_based_actions(confidence, threat_score, alert)
        ...
        for tool_result in tool_results.values():
            ...
            elif tool_result.get("reputation") == "suspicious" and "ip" in str(tool_result):
    ...
    async def _create_fallback_result(self, state: Dict[str, Any], error: str = None) -> Dict[str, Any]:
        ...
        result_state.update({
            ...
            "analysis_reasoning": [{"iteration": 1, "thought": "Fallback analysis mode activated"}],
            "tool_results": {"fallback": {"success": True, "mode": "degraded"}},
            "threat_score": fallback_confidence,
            "recommended_actions": ["Manual review recommended due to analysis limitations"],
            "triage_status": "analyzed",
            "last_updated": datetime.utcnow().isoformat(),
            "metadata": {
                **state.get("metadata", {}),
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            health_checks = {
                "llm_client": self.llm_client is not None if self.enable_llm_reasoning else True,
                "tool_orchestrator": self.tool_orchestrator is not None if self.enable_tool_orchestration else True,
                "react_reasoner": self.react_reasoner is not None if self.enable_llm_reasoning else True
            }
            ...
            if not overall_health:
                self.logger.warning(f"Health check issues: {[k for k, v in health_checks.items() if not v]}")
