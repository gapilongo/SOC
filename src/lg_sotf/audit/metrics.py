...
class MetricsCollector:
    ...
    def record_error(self, name: str, error_type: str = "error", tags: Dict[str, str] = None):
        ...
        with self._lock:
            ...
            self.error_rates[error_key] = self.error_counts[error_key] / total_operations
            
            # Record metric point
            self._record_metric(error_key, self.error_counts[error_key], 'counter', tags or {})
    ...
    def record_tool_execution(self, tool_name: str, execution_time: float, success: bool):
        ...
        with self._lock:
            ...
            self.performance_metrics[tool_name]['success_rate'] = (
                self.counters.get(f"tool_{tool_name}_success", 0) /
                max(1, self.counters.get(f"tool_{tool_name}_success", 0) + 
    ...
    def record_agent_execution(self, agent_name: str, execution_time: float, success: bool):
        ...
        with self._lock:
            ...
            self.performance_metrics[agent_name]['success_rate'] = (
                ...
                max(1, self.counters.get(f"agent_{agent_name}_success", 0) + 
    ...
    def record_workflow_execution(self, workflow_id: str, execution_time: float, success: bool):
        ...
        with self._lock:
            ...
            self.performance_metrics['workflow']['success_rate'] = (
                self.counters.get("workflow_success", 0) /
                max(1, self.counters.get("workflow_success", 0) + 
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "timing_stats": {name: asdict(stats) for name, stats in self.summaries.items()},
